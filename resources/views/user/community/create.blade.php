...
<div class="container">
    ...
    <form action="{{ route('communities.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="name" id="name" class="form-control" placeholder="Contoh: Komunitas Pemrograman" value="{{ old('name') }}" required>
        </div>

        <div class="mb-3">
            <label for="description" class="form-label">Deskripsi Komunitas</label>
            <textarea name="description" id="description" class="form-control" placeholder="Tuliskan deskripsi komunitas..." rows="4">{{ old('description') }}</textarea>
        </div>
